...
public class Crc {
    ...
    public static int calc16(byte[] val) {
        return calc16(val, 0, val.length);
    }

    public static int calc16(byte[] val, int pos, int len) {
        int index = 0;
        int crc16 = CRC16_DEFAULT_START_VALUE;

        for (int i = pos, end = pos + len;i < end;++i) {
            index = (crc16 ^ val[i]) & 0xff;
            crc16 &= 0xff00;
            crc16 |= crc16 >> 8 ^ REMAIN_TABLE_16[index];
            crc16 &= 0x00ff;
            crc16 |= REMAIN_TABLE_16[index] & 0xff00;
        }

        return crc16 & 0xffff;
    }

    public static int calc16(int crc, byte[] buf, int pos, int len) {
        int crc16 = crc & 0xffff;
        for(int i = pos, end = pos + len; i < end; i++) {
            crc16 = calc16(crc16, buf[i]);
        }
        return crc16 & 0xffff;
    }

    //倒序
    public static int calc16R(int crc, byte[] buf, int pos, int len) {
        int crc16 = crc & 0xffff;
    ...
    public static int calc16(int crc, byte val) {
